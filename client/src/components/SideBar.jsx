...
const SideBar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  const {chats,setSelectedChat,user,theme,setTheme,navigate,createNewChat,axios,setChats,fetchUserChats,setToken,token} = useAppContext()
  ...
  return (
    ...
    ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <span className='mr-2 text'>+</span>New Chat
      </button>
      ...
      {/* community images */}
        ...
        <img onClick={()=>setIsMenuOpen(false)} src={assets.close_icon} className='absolute top3- right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert' alt="" />
    </div>
  )
}
